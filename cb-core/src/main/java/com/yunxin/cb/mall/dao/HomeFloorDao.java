...
public interface HomeFloorDao extends JpaRepository<HomeFloor, Integer>, JpaSpecificationExecutor<HomeFloor>, BaseDao<HomeFloor> {
    ...
    @Query("select hf from HomeFloor hf left join fetch hf.floorCategories fct left join fetch fct.category left join fetch hf.floorCommodities fcm left join fetch fcm.commodity left join fetch hf.floorBrands fcb left join fetch fcb.brand left join fetch hf.floorAdverts fcb left join fetch fcb.advertisement where hf.floorId = ?1")
