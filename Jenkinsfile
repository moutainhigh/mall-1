        ...
        stage('Deploy') {
            steps {
                ...
                withCredentials([sshUserPrivateKey(credentialsId: 'jenkins-ssh-private-key', keyFileVariable: 'SSH_KEY_FOR_JENKINS', passphraseVariable: '', usernameVariable: 'USERNAME')]) {
                    ...
                    sh 'ssh -i ${SSH_KEY_FOR_JENKINS} -o StrictHostKeyChecking=no -p ${DOCKER_LEADER_PORT} ${USERNAME}@${DOCKER_LEADER_IP} "docker login -udengcg -pDengcg0727 192.168.0.22 && docker-compose up -f ${DOCKER_WORKSPACE}/${DOCKER_COMPOSE_FILE} -d "'
                }
