...
public class UnionPayUtils {
    ...
    public int checkSecurity(String[] valueVo) {
        ...
        if (UnionPayConf.signType.equalsIgnoreCase(map.get("signMethod"))) {
            ...
            isValid = signature.equals(md5(joinMapValue(map, '&') + md5(UnionPayConf.PAY_UNIONPAY_SECURITYKEY)));
        } else {
            isValid = verifyWithRSA(map.get("signMethod"), md5(joinMapValue(map, '&') + md5(UnionPayConf.PAY_UNIONPAY_SECURITYKEY)), signature);
        }
     ...
     *
     * @param map
     * @param signMethod 商城密钥
     * @return
     */
    private String signMap(Map<String, String> map, String signMethod) {
        if (UnionPayConf.signType.equalsIgnoreCase(signMethod)) {
            String strBeforeMd5 = joinMapValue(map, '&') + md5(UnionPayConf.PAY_UNIONPAY_SECURITYKEY);
            return md5(strBeforeMd5);
        } else {
            return signWithRSA(md5(joinMapValue(map, '&') + md5(UnionPayConf.PAY_UNIONPAY_SECURITYKEY)));
        }
    }
    ...
    private String signWithRSA(String signData) {
        ...
        String privateKeyPath = str.substring(0, num + "WEB-INF".length()) + "/MerPrK_808080201307375_20141118112327.key";
    ...
    private boolean verifyWithRSA(String algorithm, String signData, String signature) {
        String str = getClass().getResource("/").getFile().toString();
        int num = str.indexOf("WEB-INF");
        String publicKeyPath = str.substring(0, num + "WEB-INF".length()) + "/PgPubk.key";
        ...
        try {
            ...
            return signCheck.verify(Base64.getDecoder().decode(signature));
        } catch (Exception e) {
            throw new RuntimeException(e);
        } finally {
            if (pubObjectIs != null) {
    ...
    public boolean checkSign(String[] valueVo, String signMethod, String signature) {
        ...
        if (signature == null)
            return false;
        if (UnionPayConf.signType.equalsIgnoreCase(signMethod)) {
            System.out.println(">>>" + joinMapValue(map, '&') + md5(UnionPayConf.PAY_UNIONPAY_SECURITYKEY));
            System.out.println(">>>" + signature.equals(md5(joinMapValue(map, '&') + md5(UnionPayConf.PAY_UNIONPAY_SECURITYKEY))));
            return signature.equals(md5(joinMapValue(map, '&') + md5(UnionPayConf.PAY_UNIONPAY_SECURITYKEY)));
        } else {
            return verifyWithRSA(signMethod, md5(joinMapValue(map, '&') + md5(UnionPayConf.PAY_UNIONPAY_SECURITYKEY)), signature);
        }
    ...
    private String joinMapValueBySpecial(Map<String, String> map, char connector) {
        ...
        for (Map.Entry<String, String> entry : map.entrySet()) {
            ...
            if (entry.getValue() != null) {
                try {
                    b.append(java.net.URLEncoder.encode(entry.getValue(), UnionPayConf.charset));
     ...
     * String(resultInputByte,UnionPayConf.charset); } else { System.out.println("返回错误"); } } catch (Exception ex) {
